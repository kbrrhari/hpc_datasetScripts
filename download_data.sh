...
opts="--no-check-certificate -N -np -nH -R "index.html*" --cut-dirs=4"

#TC vitals
# wget http://ftp.emc.ncep.noaa.gov/wd20vxt/hwrf-init/syndat/syndat_tcvitals.2018
...
wget $opts -P ./gfs.$idate$hr -r -A .bufr_d http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.$idate$hr/
...
# grib files
# for i in {000..126..006}; do wget $opts -P ./gfs.$idate$hr http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.$idate$hr/gfs.t${hr}z.pgrb2.0p25.f$i; done
